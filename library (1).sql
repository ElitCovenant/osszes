...
--
-- Gép: 127.0.0.1:3306
-- Létrehozás ideje: 2024. Feb 06. 07:59
...
-- Eseményindítók `book`
--
DROP TRIGGER IF EXISTS `konyv_AFTER_UPDATE`;
DELIMITER $$
CREATE TRIGGER `konyv_AFTER_UPDATE` AFTER UPDATE ON `book` FOR EACH ROW BEGIN
	insert into library.kolcsonzes_tortenet (konyv_id, tag_id, datum) values (new.id, new.tag_id, NOW());
END
$$
DELIMITER ;
...
CREATE TABLE IF NOT EXISTS `user` (
  ...
  `id_account_img` int NOT NULL DEFAULT '1',
  PRIMARY KEY (`id`),
  KEY `id_rule` (`id_rule`),
...
(1, '2024-01-18 08:26:24', '2034-01-01 08:26:24', 'ambrusk@kkszki.hu', 'admin1', 1, 1),
(2, '2023-09-01 08:28:33', '2024-06-16 08:28:33', 'valami@kkszki.hu', 'proba', 2, 1);
...
ALTER TABLE `loan_history`
  ...
  ADD CONSTRAINT `fk_kolcsonzes_tortenet_tag_id` FOREIGN KEY (`tag_id`) REFERENCES `user` (`id`);
